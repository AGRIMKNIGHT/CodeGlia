...
def parse_semgrep_results(file_path: str):
    ...
    vuln_objs = []
    for i, vuln in enumerate(vulnerabilities, 1):
        # Semgrep JSON can have slightly different shapes depending on version.
        ...
        # Attempt to extract code from Semgrep output fields
        code = ''
        try:
    ...
    for idx, vuln in enumerate(vuln_objs):
        ...
        meta = vulnerabilities[idx].get('extra', {}).get('metadata', {})
        # Ignore any CWE mapping provided by Semgrep output; instead, apply dynamic lookup like Bandit
        cwe_id = "N/A"
